...
export default function Account() {
  ...
  // ── Payment success toast (shown when redirected from Instamojo) ──
  ...
  // ── Show toast if redirected from successful Instamojo payment ──
  useEffect(() => {
    const status = searchParams.get("payment");
    if (status === "success") {
      setPaymentToast(true);
      // Clean the URL so refresh doesn't show toast again
      setSearchParams({}, { replace: true });
      // Auto-dismiss after 6 seconds
      const t = setTimeout(() => setPaymentToast(false), 6000);
  ...
  const orderSubtotal = useMemo(() => Number(selectedOrder?.total_amount || 0), [selectedOrder]);
  ...
  function openOrder(o) { setSelectedOrder(o); setIsOrderModalOpen(true); }
  function closeModal()  { setIsOrderModalOpen(false); }
      ...
      {paymentToast && (
        <div className="payment-toast" role="alert">
          ...
          <div>
            ...
            <p>Your order has been placed. Check below for details.</p>
          </div>
          <button className="payment-toast-close" onClick={() => setPaymentToast(false)} aria-label="Dismiss">×</button>
        </div>
      )}
        ...
        <div className="orders-layout">
          <div className="panel">
            ...
            {ordersLoading ? (
              ...
              <div className="order-list vertical">
                {orders.map((o) => (
                  <button key={o.id} className="order-card" onClick={() => openOrder(o)} type="button">
                    <div className="row">
                      <b>Order #{o.id}</b>
          ...
          {isOrderModalOpen && (
            <div className="modalOverlay" onClick={closeModal} role="presentation">
              <div className="modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Order Details">
                ...
                {!selectedOrder ? (
                  ...
                  <>
                    ...
                    <div className="details-grid">
                      <div><div className="muted">Order ID</div><div><b>#{selectedOrder.id}</b></div></div>
                      <div><div className="muted">Status</div><div><b>{selectedOrder.status}</b></div></div>
                      <div><div className="muted">Payment</div><div><b>{selectedOrder.payment_status}</b></div></div>
                      <div>
                    ...
                    <div className="row">
                      ...
                      <span>₹{money(selectedOrder.unit_price)} × {selectedOrder.quantity}</span>
                    </div>
                    ...
                    <div className="modalFooter">
                      <button className="btnSoft" onClick={closeModal}>Close</button>
                    </div>
                  </>
                )}
              </div>
                ...
                {cart.map((it) => (
                  <div key={it.id} className="cart-item">
                    ...
                    <div className="cart-right">
                      <input type="number" min={1} value={it.qty || 1} onChange={(e) => updateQty(it.id, e.target.value)} />
